...
/// @title LocalMateJobsV3
/// @notice Arc USDC escrow with signed provider consent, evidence anchoring and dispute resolution.
contract LocalMateJobsV3 {
    ...
    IERC20V3 public immutable paymentToken;
    ...
    constructor(
        ...
        if (
            ...
            evidenceReviewPeriod < 60 ||
            evidenceReviewPeriod > 30 days
        ) revert InvalidInput();
        paymentToken = IERC20V3(usdc);
    ...
    /// @notice Payload a provider signs with personal_sign before applying.
    ...
    function assignProvider(uint256 jobId, address provider, bytes calldata providerSignature) external {
        ...
        if (
            job.status != Status.Funded ||
            block.timestamp >= job.workDeadline ||
            provider == address(0)
        ) revert InvalidState();
        if (_recover(applicationDigest(jobId, provider), providerSignature) != provider) {
    ...
    function autoRelease(uint256 jobId) external nonReentrant {
        ...
        if (
            job.status != Status.Submitted ||
            block.timestamp < uint256(job.submittedAt) + reviewPeriod
        ) revert InvalidState();
    ...
    /// @param providerShareBps Gross share of the budget awarded to the provider.
    ...
    function _safeTransfer(address to, uint256 amount) private {
        (bool success, bytes memory data) = address(paymentToken).call(
            abi.encodeCall(IERC20V3.transfer, (to, amount))
        );
    ...
    function _safeTransferFrom(address from, address to, uint256 amount) private {
        (bool success, bytes memory data) = address(paymentToken).call(
            abi.encodeCall(IERC20V3.transferFrom, (from, to, amount))
        );
        if (!success || (data.length != 0 && !abi.decode(data, (bool)))) revert TransferFailed();
    }
}
